        ...
        try (JarFile jarFile = new JarFile(appArchive)) {
            ...
            if (manifest != null) {
                ...
                return attributes.getValue("Start-Class");
            		...
            		throw new MojoExecutionException("Could not get Spring Boot start class due to error getting app manifest.");
            }
        } catch (IOException e) {
        		throw new MojoExecutionException("Could not get Spring Boot start class due to error opening app archive.", e);
        }
    }
