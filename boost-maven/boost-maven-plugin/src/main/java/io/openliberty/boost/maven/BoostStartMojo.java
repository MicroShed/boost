...
public class BoostStartMojo extends AbstractMojo {
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
    	// Check our runtime and execute maven goal for either liberty-start or
        // wildfly-start
        ...
        if (runtime.equals("wildfly")) {
        	executeMojo(
                    ...
                    goal("wildfly-start"), configuration(), executionEnvironment(project, session, pluginManager));
        } else {
        	executeMojo(
                plugin(groupId("io.openliberty.boost"), artifactId("boost-maven-plugin"), version("0.1.3-SNAPSHOT")),
                goal("liberty-start"), configuration(), executionEnvironment(project, session, pluginManager));
        }
    }
